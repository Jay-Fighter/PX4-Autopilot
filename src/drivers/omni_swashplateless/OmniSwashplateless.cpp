...
// add by jayjie
static float mapToDshotThrottle(float throttle) {
        if (!PX4_ISFINITE(throttle) || throttle <= FLT_EPSILON) {
                return 0.0f;
        }

        return math::constrain(throttle + static_cast<float>(DSHOT_THROTTLE_OFFSET),
                               static_cast<float>(DSHOT_THROTTLE_OFFSET), static_cast<float>(THROTTLE_MAX));
}
// end
...
void OmniSwashPlateLess::limit_and_update_outputs(omni_actuator_setpoint_s& output) {
        ...
        for (size_t i = 0; i < output.num_groups; i++) {
                ...
                const float lateral_norm = sqrtf(output.fx[i] * output.fx[i] + output.fy[i] * output.fy[i]);
                float us = sqrtf((lateral_norm * lateral_norm) / 2.0f) * ACTUATOR_CONTROLS_TO_DSHOT;
                // ua = 0.15*ACTUATOR_CONTROLS_TO_DSHOT;
                // us = 0;
                float phase_body = 0.0f;
                float phase_motor = 0.0f;

                if (lateral_norm > FLT_EPSILON) {
                        phase_body = atan2f(output.fy[i], output.fx[i]);

                        if (phase_body < 0.0f) {
                                phase_body += 2.0f * static_cast<float>(M_PI);
                        }
                ...
                // add by jayjie
                ua = mapToDshotThrottle(ua);
                // end

                _omni_outputs_cmd_groups.throttle_ua[i] = ua;

                // add by jayjie
                const float us_limit = ua > FLT_EPSILON
                                       ? math::min(us_2_ua_ratio_max * ua,
                                                       ua - static_cast<float>(DSHOT_THROTTLE_OFFSET))
                                       : 0.0f;
                _omni_outputs_cmd_groups.throttle_us[i] = math::constrain(us, 0.0f, math::max(us_limit, 0.0f));
                // end

                float flap = 0.0f;

                if (output.f[i] > FLT_EPSILON) {
                        const float cos_flap = math::constrain(std::fabs(output.fz[i]) / output.f[i], 0.0f, 1.0f);
                        flap = acosf(cos_flap);
                }

                _omni_outputs_cmd_groups.throttle_ctrls_flap[i] = flap;

                _omni_outputs_cmd_groups.throttle_ctrls_phase[i] = phase_motor;
...
void OmniSwashPlateLess::limit_and_update_outputs(const actuator_motors_s& actuator_motors) {
        for (size_t i = 0; i < OMNI_ACTUATOR_NUM; i++) {
                ...
                // add by jayjie
                const float throttle_ua =
                    mapToDshotThrottle(motor_throttle * static_cast<float>(ACTUATOR_CONTROLS_TO_DSHOT));
                // end
...
float OmniSwashPlateLess::getMotorPhaseFromBodyPhase(float phase_body, size_t motor_index) const {
        ...
        float phase_motor = atan2f(y_local, x_local);
...
void OmniSwashPlateLess::limit_and_update_outputs(const manual_control_setpoint_s& manual_control_setpoint) {
        ...
        // add by jayjie
        const float ua = mapToDshotThrottle(throttle_norm * ACTUATOR_CONTROLS_TO_DSHOT);
        const float us_limit = ua > FLT_EPSILON
                               ? math::min(us_2_ua_ratio_max * ua, ua - static_cast<float>(DSHOT_THROTTLE_OFFSET))
                               : 0.0f;
        const float us =
            math::constrain(roll_abs * us_2_ua_ratio_max * ua, 0.0f, math::max(us_limit, 0.0f));
        // end
        ...
        float phase_body = 0.0f;
        const bool phase_valid = phase_stick_norm > 0.05f;
        if (phase_valid) {
                phase_body = atan2f(yaw, pitch);
        ...
        for (size_t i = 0; i < OMNI_ACTUATOR_NUM; i++) {
                ...
                const float phase_motor = phase_valid ? getMotorPhaseFromBodyPhase(phase_body, i) : 0.0f;

                _omni_outputs_cmd_groups.throttle_ua[i] = active_output ? ua : 0.0f;
                _omni_outputs_cmd_groups.throttle_us[i] = active_output ? us : 0.0f;
                _omni_outputs_cmd_groups.throttle_ctrls_flap[i] = 0.0f;
                _omni_outputs_cmd_groups.throttle_ctrls_phase[i] = active_output ? phase_motor : 0.0f;
...
void OmniSwashPlateLess::limit_and_update_outputs() {
        ...
        // add by jayjie
        _single_output_cmd.throttle_ua = mapToDshotThrottle(ua_temp);
        // end
        ...
        // add by jayjie
        const float dshot_command_margin = _single_output_cmd.throttle_ua > FLT_EPSILON
                                           ? _single_output_cmd.throttle_ua - static_cast<float>(DSHOT_THROTTLE_OFFSET)
                                           : 0.0f;
        // end

        // limit us based on the throttle margin
        // add by jayjie
        if (_single_output_cmd.throttle_us > dshot_command_margin) {
                _single_output_cmd.throttle_us = dshot_command_margin;
        }
        // end
